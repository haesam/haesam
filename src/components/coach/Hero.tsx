...
export default function Hero({ onPrivacyClick }: HeroProps) {
  ...
  return (
    <section ref={sectionRef} className="grain relative flex min-h-screen flex-col overflow-hidden">
      {/* 배경: 웜 라디얼 그라디언트 */}
      <div className="pointer-events-none absolute inset-0" aria-hidden="true">
        <div className="absolute -top-40 right-[-10%] h-[560px] w-[560px] rounded-full bg-accent/10 blur-3xl" />
        <div className="absolute bottom-[-20%] left-[-8%] h-[480px] w-[480px] rounded-full bg-[#E8C9A0]/25 blur-3xl" />
      </div>
      ...
      {FLOATING_CHIPS.map((chip) => (
        ...
        >
          <span
            className="inline-block animate-float rounded-full border border-stroke bg-surface/80 px-4 py-2 text-[13px] font-medium text-muted shadow-card backdrop-blur"
      ...
      <div className="relative z-20 mx-auto grid w-full max-w-[1040px] flex-1 items-center gap-14 px-6 pb-24 pt-24 lg:grid-cols-[1.15fr_0.85fr] lg:gap-8 lg:pt-16">
        {/* 좌: 카피 + 폼 */}
        <div data-hero-text>
          <p data-hero-label className="mb-6 inline-flex items-center gap-2 rounded-full bg-accent/10 px-4 py-2 text-[13px] font-semibold text-accent-deep">
            <span className="h-1.5 w-1.5 rounded-full bg-accent" />
            무료 PDF · 5분 안에 받아보세요
          </p>

          <h1 className="font-display text-[34px] font-bold leading-[1.32] sm:text-[42px] lg:text-[46px]">
            <span className="block overflow-hidden">
              <span data-hero-line className="block">팔로워 수보다 중요한 건</span>
            </span>
            <span className="block overflow-hidden">
              <span data-hero-line className="block">
                당신이 <span className="highlight-marker text-accent-deep">어떤 사람으로</span>
              </span>
            </span>
            <span className="block overflow-hidden">
              <span data-hero-line className="block">
                <span className="highlight-marker text-accent-deep">기억되는가</span>입니다
              </span>
            </span>
          </h1>

          <p data-hero-sub className="mt-6 max-w-md text-[16px] leading-[1.7] text-muted">
            하루 1시간, 3일이면 나만의 브랜드 뼈대가 잡힙니다.
            7년차 퍼스널브랜딩 코치 유미가 만든{' '}
            <strong className="font-semibold text-ink">『3일 퍼스널브랜딩 로드맵』</strong> 워크북을
            이메일로 보내드려요.
          </p>

          <div data-hero-form className="mt-8">
      ...
      {/* 하단 마퀴 */}
      <div className="relative z-20 overflow-hidden border-t border-stroke/70 bg-surface/40 py-4 backdrop-blur-sm">
        <div className="flex w-max animate-marquee gap-10">
          {[...MARQUEE_ITEMS, ...MARQUEE_ITEMS, ...MARQUEE_ITEMS, ...MARQUEE_ITEMS].map((item, i) => (
            <span key={i} className="flex items-center gap-10 whitespace-nowrap text-[13px] font-semibold tracking-[0.14em] text-muted/70">
              {item}
